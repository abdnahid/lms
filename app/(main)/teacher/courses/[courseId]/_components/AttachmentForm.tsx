    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4 dark:bg-gray-800 dark:text-slate-300">
      <div className="font-medium flex items-center justify-between">
        ...
        <Button onClick={toggleEdit} variant="ghost">
          {isEditing && <>Cancel</>}
          {!isEditing && <IconText iconName="add" label="Add a file" />}
        </Button>
      </div>
      {!isEditing && (
        <>
          ...
          {initialData.attachments && initialData.attachments.length > 0 && (
            <div className="space-y-2">
              {initialData.attachments.map((attachment) => (
                ...
                >
                  <span className="text-lg mr-2">{getIcon("file")}</span>
                  <a
                    href={attachment.url}
                    target="_blank"
                  ...
                  {deletingId !== attachment.id && (
                    ...
                    >
                      <span className="text-lg">{getIcon("cross")}</span>
                    </button>
                  )}
                </div>
              ))}
      ...
      {isEditing && (
        <div>
          <FileUploader
            ...
            handleChange={(url, originalFilename) => {
